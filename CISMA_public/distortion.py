...
import pytorch_msssim.ssim as py_ssim
...
class Distortion(torch.nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        if config['training']['distortion_metric'] == 'MSE':
            self.dist = MSE()
        elif config['training']['distortion_metric'] == 'SSIM':
            self.dist = SSIM(data_range=1.)
        elif config['training']['distortion_metric'] == 'MS-SSIM':
    ...
    def forward(self, X, Y):
        if self.config['training']['distortion_metric'] == 'MSE':
            return self.dist.forward(X, Y).mean()  # / 255.
        elif self.config['training']['distortion_metric'] == 'SSIM':
            mse_lossfn = torch.nn.MSELoss(reduction='none')
            loss = 1 - py_ssim(X, Y, data_range=1.0, size_average=True)
            # loss += torch.mean(mse_lossfn(X, Y)).mean() * 0.2 
            return loss
# jensen-Shannon-Divergence loss
class DJSLoss(torch.nn.Module):
    """Jensen Shannon Divergence loss"""
